...
SCENARIO("there exists a is_a parser") {
    GIVEN("A delimited parser of string \"abc|efg\" (where \"abc\" and \"efg\" are left and right parens)") {
        auto parser = parsers::complete::is_a<std::string>("123456789ABCDEF"s);
...
SCENARIO("there exists a tag parser") {
    GIVEN("Tag parser of string \"hello\"") {
        auto parser = parsers::complete::tag("hello"s);
    ...
    GIVEN("Tag parser of string_view \"hello\"") {
        auto parser = parsers::complete::tag<std::string_view>("hello");
...
SCENARIO("there exists a take_while parser") {
    GIVEN("take_while is alpha") {
        auto parser = parsers::complete::take_while<std::string>([](char c) { return std::isalpha(c); });
    ...
    GIVEN("take_while_m_n 3, 6, isalpha") {
        auto parser = parsers::complete::take_while_m_n<std::string>(3, 6, [](char c) { return std::isalpha(c); });
    ...
    GIVEN("take 6 items") {
        auto parser = parsers::complete::take<std::string>(6);
